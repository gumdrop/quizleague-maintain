  ...
  val template = """
    ...
    <v-card>
      <v-card-text>
        ...
        <v-alert type="info" :icon="false" outlined border="left" text class="mt-3 transition-swing" transition="scroll-y-transition" :value="showAlert">An email has been sent with login instructions.</v-alert>
        <v-alert type="error" :icon="false" outlined border="left" text class="mt-3 transition-swing" transition="scroll-y-transition" :value="showFailure" >{{failureText}}</v-alert>
       </v-card-text>
     </v-card>
...
object PasswordLoginComponent extends Component{
  ...
  <v-layout column>
    ...
    <v-alert type="warning" :icon="false" outlined border="left" text class="mt-3 transition-swing" transition="scroll-y-transition" :value="!registered && password2 && password != password2">Passwords must match</v-alert>
    <v-alert type="error" :icon="false" outlined border="left" text class="mt-3 transition-swing" transition="scroll-y-transition" :value="showFailure">{{failureText}}</v-alert>
    <v-flex align-center style="padding-left:48%;"><v-progress-circular v-if="showProgress" indeterminate color="primary"></v-progress-circular></v-flex>
  </v-layout>
  """
...
<v-container v-bind="gridSize" fluid>
  <v-layout column>
    ...
    <v-layout row>
      ...
      <v-flex grow><v-alert type="info" :icon="false" outlined border="left" class="transition-swing" text transition="scroll-y-transition" :value="showAlert">Profile Settings Saved.</v-alert></v-flex>
    </v-layout>
  </v-layout>
</v-container>
  """
