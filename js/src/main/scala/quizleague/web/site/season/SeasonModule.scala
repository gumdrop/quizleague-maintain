...
class SeasonService(override val http: Http,
    ...
    def getResults(season:Season):Observable[js.Array[Results]] = zip(season.competitions).switchMap((c,i) => filterAndSort[Results,Fixtures](c.flatMap(_.results),((r:Results) => r.fixtures),(x:Results,y:Fixtures) => true, (r1:(Results,Fixtures),r2:(Results,Fixtures))=> r1._2.date compareTo r2._2.date))
    
    def getFixtures(season:Season) = zip(season.competitions).switchMap((c,i) => filterAndSort[Fixtures,Fixtures](c.flatMap(_.fixtures),((r:Fixtures) => r),(x:Fixtures,y:Fixtures) => true, (r1:(Fixtures,Fixtures),r2:(Fixtures,Fixtures))=> r1._2.date compareTo r2._2.date))
