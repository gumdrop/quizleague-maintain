...
object Crypto extends AES {
  def encrypt(plaintext: String, password: String, nBits: Int = 256): String = {
  ...
  /**
    ...
    */
  def decrypt(encryptedText: String, pwd: String, nBits: Int = 256): String = {
    val blockSize = 16; // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
    if (!(nBits == 128 || nBits == 192 || nBits == 256)) return ""; // standard allows 128/192/256 bit keys
    val ciphertext = BaseEncoding.base64().decode(encryptedText).map(x => byteConv(x.toLong))
