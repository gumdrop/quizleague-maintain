...
trait AES {
  ...
  def cipher(input: Array[Long], w: Array[Array[Long]]) = {
    ...
    val Nr = w.length / Nb - 1; // no of rounds: 10/12/14 for 128/192/256-bit keys
  ...
  def mixColumns(s: Array[Array[Long]], Nb: Int) = {
    for (c <- 0 until 4) {
      val a = Array.ofDim[Long](4); // 'a' is a copy of the current column from 's'
      val b = Array.ofDim[Long](4); // 'b' is a�{02} in GF(2^8)
      for (i <- 0 until 4) {
        a(i) = s(i)(c);
 ...
 *
 * @example
 *   var encr = Ctr.encrypt('big secret', 'password', 256); // encr: 'lwGl66VVwVObKIr6of8HVqJr'
 */
object Crypto extends AES {
  def encrypt(pt: String, pwd: String, nBits: Int = 256): String = {
    ...
    val ctrTxt = counterBlock.slice(0, 8)
    ...
    val ciphertxt = Array.ofDim[Long](blockCount, blockSize); // ciphertext as array of strings
    ...
    for (b <- 0 until blockCount) {
      ...
      val blockLength = if (b < blockCount - 1) blockSize else (plaintext.length - 1) % blockSize + 1;
      val cipherChar = Array.ofDim[Long](blockLength);
    ...
    Base64.getEncoder.encodeToString(longs.map(_.toByte))
  }
   ...
   *
   * @example
   *   var decr = Ctr.encrypt('lwGl66VVwVObKIr6of8HVqJr', 'password', 256); // decr: 'big secret'
   */
  def decrypt(et: String, pwd: String, nBits: Int = 256): String = {
    val blockSize = 16; // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
    if (!(nBits == 128 || nBits == 192 || nBits == 256)) return ""; // standard allows 128/192/256 bit keys
    val ciphertext = Base64.getDecoder.decode(et).map(x => byteConv(x.toLong))
    ...
    val nBlocks = Math.ceil((ciphertext.length - 8) / blockSize).toInt + 1
